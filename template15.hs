...
getLocals sts
  = [lcs | lcs@(a, (sc, val)) <- sts, sc == Local]

getGlobals :: State -> State
getGlobals sts
  = [lcs | lcs@(a, (sc, val)) <- sts, sc == Global]

assignArray :: Value -> Value -> Value -> Value -- Might be sth wrong here
...
assignArray (A as) (I i) (I v)
  = A ((i, v) : [a | a@(ia, va) <- as, ia /= i ])

updateVar :: (Id, Value) -> State -> State  -- Might be sth wrong here
updateVar (i, v) st
  | filter ((i == ).fst) st == [] = (i, (Local, v)) : st
  | otherwise                     = (i, (sta, v)) : (filter ((i /= ).fst) st)
  where
    (sta, va) = lookUp i st
...
applyOp Less (I a) (I b)
  = if a < b then (I 1) else (I 0)
applyOp Equal (I a) (I b)
  = if a == b then (I 1) else (I 0)
applyOp Index (A as) (I i)
  | lookup i as == Nothing = I 0
  | otherwise              = I (lookUp i as)

bindArgs :: [Id] -> [Value] -> State
-- Pre: the lists have the same length
bindArgs ids vals
  = zip ids [(Local, val) | val <- vals]

evalArgs :: [Exp] -> [FunDef] -> State -> [Value]
evalArgs exps fs st
  = map (evalArgs' fs st) exps
  where
    evalArgs' funcs sta exp = eval exp funcs sta
...
eval (Var id) _ st
  = getValue id st
eval (Cond cnd t f) fs st
  | eval cnd fs st == (I 1) = eval t fs st
  | otherwise         = eval f fs st
eval (OpApp op a b) fs st
  = applyOp op (eval a fs st) (eval b fs st)
eval (FunApp f exps) fs st
  = eval e  fs st'
  where
    (as, e) = lookUp f fs
    vs = evalArgs exps fs st
...
executeStatement (Assign id exp) fs ps st
  = updateVar (id, eval exp fs st) st
executeStatement (AssignA id i exp) fs ps st
  = updateVar (id, assignArray (getValue id st) (eval i fs st) (eval exp fs st)) st
executeStatement (If exp blt blf) fs ps st
  | eval exp fs st == (I 1) = executeBlock blt fs ps st
  | otherwise               = executeBlock blf fs ps st
executeStatement (While exp bl) fs ps st
  | eval exp fs st == (I 1) = executeStatement (While exp bl) fs ps st'
  | otherwise               = st
  where
    st' = executeBlock bl fs ps st
executeStatement (Call x p exps) fs ps st
  | x == ""   = returnS
  | otherwise = updateVar (x, getValue "$res" st') returnS
  where
    globals = getGlobals st
    (vars, bl) = lookUp p ps
    vals = evalArgs exps fs st
    localSt = bindArgs vars vals
    st' = executeBlock bl fs ps (localSt ++ globals)
    returnS = (getLocals st) ++ (getGlobals st')
executeStatement (Return e) fs ps st
  = updateVar ("$res", eval e fs st) (getGlobals st)  

executeBlock :: Block -> [FunDef] -> [ProcDef] -> State -> State
-- Pre: All code blocks and associated statements are well formed
executeBlock [] _ _ st
  = st
executeBlock (stm : stms) fs ps st
  = executeBlock stms fs ps st'
  where
    st' = executeStatement stm fs ps st
